...
# Diesel ₹/km is derived from the pump price ÷ mileage so it stays correct as fuel
# price moves. Insurance ₹420/bus/day calibrated to the GUT FY25-26 actuals.
DIESEL_PER_LITRE = 100.0                # ₹/litre — current pump price
MILEAGE_KMPL     = 5.68                 # km/litre — fleet average (GUT FY25-26 actual)
OWN_DRIVER, OWN_MAINT = 762, 1624
OWN_DIESEL_KM = round(DIESEL_PER_LITRE / MILEAGE_KMPL, 2)   # = ₹17.61/km
OWN_INSURANCE = 420                    # ₹/owned bus/day — insurance & taxes (GUT FY25-26 actual)
    ...
    with open(path, encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            try:
                ...
                continue
            out.append({"name": (r.get("Name of Stop") or "Stop").strip(), "lat": lat, "lng": lng})
    return out

ROAD_MATRIX_FILE = "data/road_matrix.json"   # built once by build_road_matrix.py

def _load_cached_matrix(coords):
    """Use the cached Google road matrix if it's present AND lines up with these coords
    (same node count + order, depot at 0). Returns (km, mins) or None to fall back."""
...
def build_matrices(coords):
    cached = _load_cached_matrix(coords)
    if cached:
        return cached
...
def main():
    ...
    # --- arc cost: owned pays diesel/km, rental pays a tiny epsilon (flat tariff added as fixed)
    def make_cost(per_km):
        def cb(i, j):
            return int(round(km[mgr.IndexToNode(i)][mgr.IndexToNode(j)] * per_km))
        return routing.RegisterTransitCallback(cb)
    own_cost  = make_cost(OWN_DIESEL_KM)
    rent_cost_cb = make_cost(RENT_EPS_KM)
    for v, f in enumerate(fleet):
        routing.SetArcCostEvaluatorOfVehicle(own_cost if f["own"] else rent_cost_cb, v)
        # Owned driver+maint are SUNK (paid whether the bus runs or not), so an owned bus's
        # marginal cost is only diesel -> fixed cost 0 => owned filled first (sunk-cost-first).
        # A rental is a fresh ₹1700 only if called. True ₹ is recomputed afterwards for reporting.
    ...
    # --- ride time (travel + service), generous ceiling so buses can carry many stops
    def time_cb(i, j):
    ...
    params.time_limit.FromSeconds(args.seconds)
    ...
    status = {0:"NOT_SOLVED",1:"SUCCESS",2:"FAIL",3:"FAIL_TIMEOUT",4:"INVALID"}.get(routing.status(), routing.status())
    if not sol:
        print(f"No solution found.  status={status}")
        return

    # Re-sequence each bus's stops FARTHEST-FIRST: the bus deadheads out to the
    # farthest stop, then picks everyone up working back toward the factory (nearest-
    # neighbour). No backtrack -> the longest passenger ride ~= the farthest stop's
    # distance, not a zig-zag. (Same approach as the dashboard engine.)
    ...
    for v in range(V):
        ...
        seq = resequence(nodes)                       # farthest -> ... -> nearest
        deadhead = km[0][seq[0]]                       # empty leg out to the farthest stop
        inbound  = sum(km[seq[i]][seq[i+1]] for i in range(len(seq)-1)) + km[seq[-1]][0]
        dist = deadhead + inbound
        # longest passenger ride = farthest rider's journey home = the inbound leg
        ride = inbound / SPEED_KMPH * 60 + SERVICE_MIN * len(seq)
    ...
    # ---- detail of the longest-ride route: ordered stops + lat/lng + maps link ----
    worst = max(routes, key=lambda r: r["ride"])
    print(f"\n================  LONGEST ROUTE: {worst['name']}  "
          f"({worst['stops']} stops, {worst['km']:.1f} km, {worst['ride']:.0f} min)  ================")
    print(f"  DEPOT (factory)          {DEPOT[0]:.6f}, {DEPOT[1]:.6f}")
    for k, node in enumerate(worst["seq"], 1):
        s = stops[node - 1]
        print(f"  {k}. {s['name']:<28} {s['lat']:.6f}, {s['lng']:.6f}")
    waypts = "|".join(f"{stops[n-1]['lat']},{stops[n-1]['lng']}" for n in worst["seq"])
    print(f"\n  Google Maps: https://www.google.com/maps/dir/?api=1&origin={DEPOT[0]},{DEPOT[1]}"
          f"&destination={DEPOT[0]},{DEPOT[1]}&waypoints={waypts}&travelmode=driving")
    ...
    out = {
        ...
        "routes": [{"name": r["name"], "type": "own" if r["own"] else "rent", "cap": r["cap"],
                    ...
                    "km_to_farthest": round(km[0][r["seq"][0]], 1), "km_to_last": round(km[0][r["seq"][-1]], 1),
