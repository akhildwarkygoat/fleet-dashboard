...
export default function OptimiserTab({ t, toast }) {
  ...
  // regenerated by the Prev-route rebuild) — replaces the stale seed, preserves ids/edits
  useEffect(() => {
    fetch("/merged_stops.json?ts=" + Date.now())
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (d && d.stops && d.stops.length && store.syncStopsFromErp) { const res = store.syncStopsFromErp(d.stops); if (res.synced) refresh(); } })
      .catch(() => {});
  }, []);
