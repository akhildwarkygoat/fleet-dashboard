...
def route_metrics(stop_nodes, km, mins, dem):
    """km, ride(min), riders for an (unordered) set of stop node-ids. Depot is node 0.
    `dem` is a node-indexed demand array (dem[0]=depot=0), so riders can vary per stop."""
    ...
    return dist, ride, sum(dem[n] for n in seq), seq
...
def generate_pool(n_stops, km, mins, dem, owned_cap, rent_cap, hard_ride, seed_routes=None):
    ...
    # with variable demand, the tightest (smallest) per-stop demand bounds the most stops a
    # bus could hold; consider() still rejects any window whose actual riders exceed the cap.
    min_dem = min(dem[s] for s in stops)
    max_owned = max(1, (owned_cap // min_dem))
    max_rent = max(1, (rent_cap // min_dem))
    ...
    def consider(stop_set, is_owned):
        ...
        riders = sum(dem[s] for s in stop_set)
        if riders > cap:
            return
        key = (is_owned, frozenset(stop_set))
        if key in seen:
            return
        dist, ride, riders, seq = route_metrics(stop_set, km, mins, dem)
        if ride > hard_ride:
            return
        seen.add(key)
...
def main():
    ...
    # TRIAL: add a SMALL random deviation to each stop's headcount around the base
    # (--riders/n_stops ≈ 8), seeded/reproducible. This jitters how many people board at a
    # stop — and therefore the riders on each bus — by ±demand-dev, WITHOUT changing the fleet.
    # It shows how sensitive the plan is to normal day-to-day ridership wobble.
    # --demand-dev 0 keeps the original flat count.
    ap.add_argument("--demand-dev", type=int, default=0)
    ...
    d0 = max(1, math.ceil((args.riders / n_stops) * (1 - O.ABSENTEE + O.BUFFER)))  # constant = 8
    if args.demand_dev > 0:                              # small +/- jitter, centred on d0 (trial)
        # keeps the TOTAL ~unchanged (symmetric jitter) so the fleet/bus count stays put — we
        # only wobble how many board at each stop, not how many buses are needed.
        dem_list = [max(1, d0 + rng.randint(-args.demand_dev, args.demand_dev)) for _ in range(n_stops)]
        demand_mode = f"jitter {d0}±{args.demand_dev} seed {args.seed}"
    else:                                                # original: flat count
        dem_list = [d0] * n_stops
        demand_mode = f"constant {d0}/stop"
    total_dem = sum(dem_list)
    dem_avg = total_dem / n_stops
    ...
    demand = [0] + dem_list                              # node-indexed (depot = 0)
    N = n_stops + 1

    print(f"Generating candidate routes for {n_stops} stops, {total_dem} riders "
          f"({demand_mode}: avg {dem_avg:.1f}/stop, range {min(dem_list)}-{max(dem_list)}), "
          f"caps owned {owned_cap} / rent {rent_cap}, hard ride {args.max_ride} min ...")
    ...
    pool = generate_pool(n_stops, km, mins, demand, owned_cap, rent_cap, args.max_ride, seed_routes=seed)
    ...
    out = {
        ...
        "assumptions": {
            ...
            "working_days": O.WORKING_DAYS, "cap_leniency": O.CAP_LENIENCY,
            "demand_per_stop": round(dem_avg, 1), "demand_mode": demand_mode,
            "demand_range": [min(dem_list), max(dem_list)],
